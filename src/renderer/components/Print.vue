<template>
  <div class="contents">
    <span class="printbar" v-if="printbar">
      <a class="print-option" @click.prevent="print">Print</a>
      <a class="print-option" @click.prevent="save">Save to PDF</a>
    </span>

    <div
      id="print-target"
      v-if="printType !== 'clipper'"
      class="print-target hljs"
      v-html="printContents"
    ></div>

    <span v-html="printContents" class="clipper-url" v-if="printType === 'clipper'"></span>
    <div id="clipper-window" v-if="printType === 'clipper'">
      <webview id="clipper-webview" :src="indexPath" autosize="on" disablewebsecurity></webview>
    </div>
...
export default {
  ...
  mounted () {
    document.getElementsByClassName('app-window')[0].className = 'print-window'
  ...
  computed: {
    printContents () {
      ...
      return this.$store.state.mainWindow.printHtml
    },
    printType () {
      return this.$store.state.mainWindow.activeNote.metadata.type
    },
    attachmentPath () {
      if (this.$store.state.mainWindow.activeNote.notePath) {
        return path.join(this.$store.state.mainWindow.activeNote.notePath, 'attachments')
      } else {
        return ''
      }
    },
  ...
  methods: {
    print () {
      this.printbar = false
      let target
      if (this.printType === 'clipper') {
        target = this.webview
      } else {
        target = remote.getCurrentWindow().webContents
      }
      target.print()
      setTimeout(() => {
        this.printbar = true
    ...
    save () {
      ...
      target.printToPDF({
        ...
        if (err) alert('Could not save file')
        // Get save path
        let savePath = remote.dialog.showSaveDialog(remote.getCurrentWindow(), {
          title: 'Save to PDF',
          buttonLabel: 'Save',
          filters: [
        ...
        if (typeof savePath !== 'undefined' && savePath) {
          fs.writeFile(savePath, data, (err) => {
            log.error('Error occurred while saving pdf ' + err)
            if (err) alert('Error occurred while saving pdf ' + err)
            this.printbar = true
          })
