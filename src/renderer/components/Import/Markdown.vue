...
<template>
  <div class="container is-fluid importer">
    <div class='columns'>
      <div class='column'>
        <p>
          This tool will import data from a directory containing markdown files (.md).  It will not import any attachments, each markdown file will create a single note.
        </p>
      </div>
    </div>
    ...
    <div class='columns'>
      <div class="column is-12">
        <h1 class="title">Markdown Files</h1>
      </div>
    </div>
    ...
    <div class='columns'>
      <div class='column instruction is-half'>
        <p>
          Where is the folder containing the markdown files?
        </p>
      </div>
...
import MarkdownFile from '../../collection/import/markdown.js'
export default {
  name: 'text-file',
  data: function () {
    return {
      textFolderPath: '',
      notebookName: 'Markdown File Import',
      recursive: false
    }
  },
  computed: {
  ...
  methods: {
    ...
    runImport () {
      ...
      let output = new MarkdownFile(this.textFolderPath, outputFolder, this.recursive).convert()
      if (output.completed) {
